...
def get_payping_transactions(from_date, to_date):
    ...
    len_data = 50
    while len_data == 50:
        resp = requests.post(url="https://api.payping.ir/v1/report/TransactionReport", headers=headers, json=request_json, timeout=30)
        data = data + resp.json()
...
def make_payping_transaction(res, fee, payping_account, transaction_dic=None):
    amount = res["amount"]
    is_paid = res["isPaid"]
